...
- **Two units, because a user manager is not a system manager.** `IPAddressDeny=` is "only available for system services, not for per-user services" (systemd.resource-control(5)), and the filesystem-namespacing settings are unavailable to user services without `PrivateUsers=`. So `contrib/systemd/system/` carries the full unit and `contrib/systemd/user/` carries the seccomp-and-limits subset, which states in its own header that it is not enforcing the clearnet lock. `make install` picks by prefix. Shipping one file that asks for what a user manager cannot give is not a degradation — systemd fails the service instead.
