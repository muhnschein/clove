...
- The daemon restricts *itself* as it passes lifecycle phases, OpenSSH-style. After initialization (config read, data directory opened, SAM connected, control socket bound), it applies:
  ...
  - A syscall number is not always the whole policy, so four calls are restricted by **argument** as well: `socket(2)` to `AF_INET`/`SOCK_STREAM` (the type half is where Layer 2 closes datagram egress, which Landlock's `ConnectTcp` does not reach), `ioctl(2)` to `FIONBIO` (closing `SIOCGIFCONF` interface enumeration), and `mmap(2)`/`mprotect(2)` to W^X.
  ...
  - **Reported, and optionally required.** Best-effort means the interesting failure is silent, so the verdict is held for the life of the process and served from `GET /v1/status`, and `sandbox require` (`clove.conf(5)`) turns an incomplete confinement into a refusal to start. The default stays permissive; the point is that an operator can see which case they are in and say which they accept.
...
- **Calendar versioning** ([calver.org](https://calver.org/)): a release is named for the month it was cut in — full year, zero-padded month, `YYYY.0M`, e.g. *2026.08*. A second release inside one month appends a counter (*2026.08.1*). No major/minor/patch numbers: a release name says when, not what it promises to be compatible with. The compatibility promise belongs to the state format, which carries its own integer version (`docs/STATE-FORMAT.md`), and to the `/v1/` API path.
- **One release, two spellings, one test.** `clove_core::VERSION` is the name everything prints; `Cargo.toml` carries the same release as `2026.8.0`, because semver forbids the leading zero in `08`. A test in `clove-core` fails when a bump touches only one of them. Cutting a release is: bump both, refresh the version in the man-page examples, tag `v2026.08`.
