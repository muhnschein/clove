...
- The daemon restricts *itself* as it passes lifecycle phases, OpenSSH-style. After initialization (config read, data directory opened, SAM connected, control socket bound), it applies:
  ...
  - A syscall number is not always the whole policy, so five calls are restricted by **argument** as well: `socket(2)` to `AF_INET`/`SOCK_STREAM` (the type half is where Layer 2 closes datagram egress, which Landlock's `ConnectTcp` does not reach), `ioctl(2)` to `FIONBIO` (closing `SIOCGIFCONF` interface enumeration), `mmap(2)`/`mprotect(2)` to W^X, and `prctl(2)` to `PR_SET_NAME`, so every thread can carry a name without the rest of `prctl` (dumpability, `no_new_privs`, the speculation controls) coming with it.
